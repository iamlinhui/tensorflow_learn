...
y_data = tf.compat.v1.placeholder(tf.float32, [None, 10])  # 模型参数
W = tf.Variable(tf.compat.v1.truncated_normal([784, 10], stddev=0.1), name="W")
b = tf.Variable(tf.constant(0.1, shape=[10]), name="b")
# 构造模型
y_pred = tf.nn.softmax(tf.matmul(x_data, W) + b)
# 损失函数
cost = tf.reduce_mean(-tf.reduce_sum(y_data * tf.compat.v1.log(y_pred), reduction_indices=1))
# 优化
...
print()
"Accuracy:", accuracy.eval({x_data: train_data.test.images, y_data: train_data.test.labels})
